...
- **Reranker weights baked into the image:** The BAAI/bge-reranker-base weights (~1.1 GB) are downloaded at *build* time into `HF_HOME=/opt/hf`, not fetched on first run. Two reasons. First, `/opt` sits outside the mounted `ragdata:/data` volume — a pre-existing non-empty volume shadows image content at its mount path, which would have hidden the weights. Second, and more important, `reranker.py` catches a model-load failure and falls back to lexical scoring *silently*, so a failed download on an ephemeral filesystem would present as a healthy boot with cross-encoder reranking quietly disabled. Baking the weights in makes that failure impossible at runtime and keeps cold start independent of network egress.
